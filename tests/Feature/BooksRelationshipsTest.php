<?php

namespace Tests\Feature;

use App\Exceptions\Handler;
use App\Models\Author;
use App\Models\Book;
use App\Models\User;
use Illuminate\Database\QueryException;
use Illuminate\Foundation\Testing\DatabaseMigrations;
use Illuminate\Http\Request;
use Illuminate\Testing\TestResponse;
use Laravel\Passport\Passport;
use Tests\TestCase;

class BooksRelationshipsTest extends TestCase
{
//    use DatabaseMigrations;

    /**
     * @test
     */
    public function it_can_modify_relationships_to_authors_and_add_new_relationships()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->count(10)->create();

        $user = User::factory()->create();
        Passport::actingAs($user);

        $this->patchJson('/api/v1/books/1/relationships/authors', [
            "data" => [
                [
                    "id" => "5",
                    "type" => "authors"
                ], [
                    "id" => "6",
                    "type" => "authors"
                ],
            ]
        ], [
            "Accept" => "application/vnd.api+json",
            "Content-Type" => "application/vnd.api+json",
        ])->assertStatus(204);

        $this->assertDatabaseHas('author_book', [
            'author_id' => 5,
            'book_id' => 1
        ])->assertDatabaseHas('author_book', [
            'author_id' => 6,
            'book_id' => 1
        ]);
    }

    /**
     *
     * @test
     */
    public function a_relationship_link_to_authors_returns_all_related_authors_as_resource_id_object()
    {
        $books = Book::factory()->create();
        $authors = Author::factory()->count(3)->create();
        $books->authors()->sync($authors->pluck("id"));

        $user = User::factory()->create();
        Passport::actingAs($user);

        $this->getJson("/api/v1/books/1/relationships/authors", [
            "Accept" => "application/vnd.api+json",
            "Content-type" => "application/vnd.api+json",
        ])->assertStatus(200)
            ->assertJson([
                "data" => [
                    [
                        "id" => 1,
                        "type" => "authors"
                    ], [
                        "id" => 2,
                        "type" => "authors"
                    ], [
                        "id" => 3,
                        "type" => "authors"
                    ],
                ]
            ]);
    }

    /**
     * @test
     */
    public function it_can_modify_relationships_to_authors_and_remove_relationships()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->count(10)->create();

        $user = User::factory()->create();
        Passport::actingAs($user);

        $this->patchJson('/api/v1/books/1/relationships/authors', [
            "data" => [
                [
                    "id" => "1",
                    "type" => "authors"
                ], [
                    "id" => "2",
                    "type" => "authors"
                ], [
                    "id" => "5",
                    "type" => "authors"
                ],
            ]
        ], [
            "Accept" => "application/vnd.api+json",
            "Content-Type" => "application/vnd.api+json",
        ])->assertStatus(204);

        $this->assertDatabaseHas('author_book', [
            'author_id' => 1,
            'book_id' => 1
        ])->assertDatabaseHas('author_book', [
            'author_id' => 2,
            'book_id' => 1
        ])->assertDatabaseHas('author_book', [
            'author_id' => 5,
            'book_id' => 1
        ]);
    }

    /**
     * @test
     */
    public function it_can_remove_all_relationships_to_authors_with_an_empty_collection()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->count(10)->create();
        $book->authors()->sync($authors->pluck("id"));

        $user = User::factory()->create();
        Passport::actingAs($user);

        $this->patchJson('/api/v1/books/1/relationships/authors', [
            "data" => []
        ], [
            "Accept" => "application/vnd.api+json",
            "Content-Type" => "application/vnd.api+json",
        ])->assertStatus(204);
    }

    /**
     * @test
     */
    public function it_returns_a_404_not_found_when_trying_to_add_relationship_to_a_non_existing()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->count(5)->create();

        $user = User::factory()->create();
        Passport::actingAs($user);

        $this->patchJson('/api/v1/books/1/relationships/authors', [
            "data" => [
                [
                    "id" => "5",
                    "type" => "authors"
                ], [
                    "id" => "6",
                    "type" => "authors"
                ],
            ]
        ], [
            "Accept" => "application/vnd.api+json",
            "Content-Type" => "application/vnd.api+json",
        ])->assertStatus(404)
            ->assertJson([
                "errors" => [
                    [
                        "title" => 'Not Found Http Exception',
                        'details' => 'Resource Not Found'
                    ]
                ]
            ]);

    }

    /**
     * Validation on Relationship attributes
     */

    /**
     * @test
     */
    public function it_validates_that_the_id_member_is_given_when_updating_a_relationship()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->create();
        $user = User::factory()->create();
        Passport::actingAs($user);
        $this->patchJson('/api/v1/books/1/relationships/authors', [
            'data' => [
                [
                    'type' => 'authors',
                ],
            ]
        ], [
            'accept' => 'application/vnd.api+json',
            'content-type' => 'application/vnd.api+json',
        ])->assertStatus(422)->assertJson([
            'errors' => [
                [
                    'title' => 'Validation Errors',
                    'details' => 'The data.0.id field is required.',
                    'source' => [
                        'pointer' => '/data/0/id',
                    ]
                ]
            ]
        ]);
    }

    /**
     * @test
     */
    public function it_validates_that_the_id_member_is_string_when_updating_a_relationship()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->create();
        $user = User::factory()->create();
        Passport::actingAs($user);
        $this->patchJson('/api/v1/books/1/relationships/authors', [
            'data' => [
                [
                    "id" => 5,
                    'type' => 'authors',
                ],
            ]
        ], [
            'accept' => 'application/vnd.api+json',
            'content-type' => 'application/vnd.api+json',
        ])->assertStatus(422)->assertJson([
            'errors' => [
                [
                    'title' => 'Validation Errors',
                    'details' => 'The data.0.id must be a string.',
                    'source' => [
                        'pointer' => '/data/0/id',
                    ]
                ]
            ]
        ]);
    }

    /**
     * @test
     */
    public function it_validates_that_the_type_member_is_given_when_updating_a_relationship()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->create();
        $user = User::factory()->create();
        Passport::actingAs($user);
        $this->patchJson('/api/v1/books/1/relationships/authors', [
            'data' => [
                [
                    'id' => "5",
                ],
            ]
        ], [
            'accept' => 'application/vnd.api+json',
            'content-type' => 'application/vnd.api+json',
        ])->assertStatus(422)->assertJson([
            'errors' => [
                [
                    'title' => 'Validation Errors',
                    'details' => 'The data.0.type field is required.',
                    'source' => [
                        'pointer' => '/data/0/type',
                    ]
                ]
            ]
        ]);
    }

    /**
     * @test
     */
    public function it_validates_that_the_id_member_has_value_of_authors_when_updating_a_relationship()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->create();
        $user = User::factory()->create();
        Passport::actingAs($user);
        $this->patchJson('/api/v1/books/1/relationships/authors', [
            'data' => [
                [
                    "id" => "5",
                    'type' => 'books',
                ],
            ]
        ], [
            'accept' => 'application/vnd.api+json',
            'content-type' => 'application/vnd.api+json',
        ])->assertStatus(422)->assertJson([
            'errors' => [
                [
                    'title' => 'Validation Errors',
                    'details' => 'The selected data.0.type is invalid.',
                    'source' => [
                        'pointer' => '/data/0/type',
                    ]
                ]
            ]
        ]);
    }

    /**
     * @test
     */
    public function it_can_get_all_related_authors_as_resource_objects_from_related_link()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->count(3)->create();
        $book->authors()->sync($authors->pluck('id'));
        $user = User::factory()->create();
        Passport::actingAs($user);
        $this->getJson('/api/v1/books/' . $book->id . '/authors', [
            "Accept" => "application/vnd.api+json",
        ])
            ->assertStatus(200)
            ->assertJson([
                'data' => [
                    [
                        "id" => $authors[0]->id,
                        "type" => "authors",
                        "attributes" => [
                            'name' => $authors[0]->name,
                            'created_at' => date("Y-m-d H:i:s", strtotime($authors[0]->created_at)),
                            'updated_at' => date("Y-m-d H:i:s", strtotime($authors[0]->updated_at)),
                        ]
                    ],
                    [
                        "id" => $authors[1]->id,
                        "type" => "authors",
                        "attributes" => [
                            'name' => $authors[1]->name,
                            'created_at' => date("Y-m-d H:i:s", strtotime($authors[1]->created_at)),
                            'updated_at' => date("Y-m-d H:i:s", strtotime($authors[1]->updated_at)),
                        ]
                    ],
                    [
                        "id" => $authors[2]->id,
                        "type" => "authors",
                        "attributes" => [
                            'name' => $authors[2]->name,
                            'created_at' => date("Y-m-d H:i:s", strtotime($authors[2]->created_at)),
                            'updated_at' => date("Y-m-d H:i:s", strtotime($authors[2]->updated_at)),
                        ]
                    ],
                ]
            ]);
    }

    /**
     * @test
     */
    public function it_includes_related_resource_objects_when_an_include_query_param_is_given()
    {
        $book = Book::factory()->create();
        $authors = Author::factory()->count(3)->create();
        $book->authors()->sync($authors->pluck('id'));
        $user = User::factory()->create();
        Passport::actingAs($user);
        $this->getJson('/api/v1/books/1?include=authors', [
            'accept' => 'application/vnd.api+json',
            'content-type' => 'application/vnd.api+json',
        ])
            ->assertStatus(200)
            ->assertJson([
                'data' => [
                    'id' => '1',
                    'type' => 'books',
                    'relationships' => [
                        'authors' => [
                            'links' => [
                                'self' => route(
                                    'books.relationships.authors',
                                    ['id' => $book->id]
                                ),
                                'related' => route(
                                    'books.authors',
                                    ['id' => $book->id]
                                ),
                            ],
                            'data' => [
                                [
                                    'id' => (string) $authors->get(0)->id,
                                    'type' => 'authors'
                                ],
                                [
                                    'id' => (string) $authors->get(1)->id,
                                    'type' => 'authors'
                                ]
                            ]
                        ]
                    ]
                ],
                'included' => [
                    [
                        "id" => $authors[0]->id,
                        "type" => "authors",
                        "attributes" => [
                            'name' => $authors[0]->name,
                            'created_at' => date("Y-m-d H:i:s", strtotime($authors[0]->created_at)),
                            'updated_at' => date("Y-m-d H:i:s", strtotime($authors[0]->updated_at)),
                        ]
                    ],
                    [
                        "id" => $authors[1]->id,
                        "type" => "authors",
                        "attributes" => [
                            'name' => $authors[1]->name,
                            'created_at' => date("Y-m-d H:i:s", strtotime($authors[1]->created_at)),
                            'updated_at' => date("Y-m-d H:i:s", strtotime($authors[1]->updated_at)),
                        ]
                    ],
                    [
                        "id" => $authors[2]->id,
                        "type" => "authors",
                        "attributes" => [
                            'name' => $authors[2]->name,
                            'created_at' => date("Y-m-d H:i:s", strtotime($authors[2]->created_at)),
                            'updated_at' => date("Y-m-d H:i:s", strtotime($authors[2]->updated_at)),
                        ]
                    ],
                ]
            ]);
    }
}
